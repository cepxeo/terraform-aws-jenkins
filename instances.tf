...
#Get Linux AMI ID using SSM Parameter endpoint
data "aws_ssm_parameter" "linuxAmi1" {
...
resource "aws_instance" "jenkins-worker" {
  ...
  ami                         = data.aws_ssm_parameter.linuxAmi1.value
  instance_type               = var.instance-type
  key_name                    = aws_key_pair.worker-key.key_name
  associate_public_ip_address = true
  vpc_security_group_ids      = [aws_security_group.jenkins-sg-1.id]
  subnet_id                   = aws_subnet.subnet_1_1.id
